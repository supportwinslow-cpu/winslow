...
export default function ProductClient({ slug }) {
    ...
    const isBracketProduct =
        product.name?.toLowerCase().includes("bracket") ||
        product.slug?.toLowerCase().includes("bracket") ||
        product.variants?.some((variant) =>
            variant.name?.toLowerCase().includes("bracket")
        );

    const originalPrice = isBracketProduct ? 4800 : 3800;
    const salePrice = isBracketProduct ? 3120 : 2470;
    ...
    const handleAddToCart = () => {
        addToCart(cartProduct, quantity, product.brand);
    };
                    ...
                    <div className="grid gap-8 rounded-[2.2rem] bg-white p-4 sm:p-6 lg:grid-cols-2 lg:p-8">
                        ...
                        <div className="flex flex-col justify-center">
                            ...
                            <div className="mt-5 flex flex-wrap items-center gap-3">
                                <span className="text-xl font-black text-gray-400 line-through">
                                    ₹{originalPrice}
                                </span>

                                <span className="text-5xl font-black text-[#2F2FE4]">
                                    ₹{salePrice}
                                </span>

                                <span className="rounded-full bg-green-50 px-4 py-2 text-xs font-black uppercase tracking-wide text-green-600">
                                    Save 35%
                                </span>
                            </div>
                ...
                <section className="mt-20">
                    ...
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                        {relatedProducts.map((item, index) => {
                            ...
                            const isRelatedBracketProduct =
                                item.name?.toLowerCase().includes("bracket") ||
                                item.slug?.toLowerCase().includes("bracket") ||
                                item.variants?.some((variant) =>
                                    variant.name?.toLowerCase().includes("bracket")
                                );

                            const relatedOriginalPrice = isRelatedBracketProduct ? 4800 : 3800;
                            const relatedSalePrice = isRelatedBracketProduct ? 3120 : 2470;

                            return (
                                <Link
                                    key={item.id}
                                    href={`/shop/parcel-tray/${item.slug}`}
                                    className="group"
                                >
                                    <div className="relative h-full overflow-hidden rounded-4xl border border-[#2F2FE4]/10 bg-white p-3 shadow-[0_20px_60px_rgba(47,47,228,0.10)] transition-all duration-500 hover:-translate-y-3 hover:border-[#2F2FE4]/45 hover:shadow-[0_30px_90px_rgba(47,47,228,0.20)]">
                                        ...
                                        <div className="p-4 text-center">
                                            ...
                                            <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
                                                ...
                                                <span className="rounded-full bg-green-50 px-2.5 py-1 text-[9px] font-black uppercase tracking-wide text-green-600">
                                                    35% OFF
                                                </span>
                                            </div>
                                        </div>
